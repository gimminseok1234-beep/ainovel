    ...
    <div className="w-full max-w-4xl mx-auto p-6 pb-32 animate-in fade-in duration-300">
      ...
      <div className="mb-6 p-4 bg-gray-800 border border-gray-700 rounded-xl shadow-lg">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
             ...
             >
                 {(Array.isArray(AI_MODELS) ? AI_MODELS : []).map(model => (
         ...
         <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="space-y-6">
                    ...
                    <div className="space-y-2"><label className="text-sm font-medium text-gray-300 flex items-center gap-2">문체 설정 (Style)</label><select className="w-full bg-gray-900 border border-gray-700 rounded-lg p-2.5 text-sm text-gray-200 outline-none focus:border-indigo-500 mb-2" value={settings.activeStyleId || ''} onChange={handleStyleSelect}><option value="">(기본 문체)</option>{(Array.isArray(savedStyles) ? savedStyles : []).map(s => (<option key={s.id} value={s.id}>{s.name}</option>))}</select>{hasStyle ? (<div className="p-3 border rounded-lg text-xs flex items-center gap-2 bg-green-900/20 border-green-500/30 text-green-400"><CheckCircle2 size={14} /> <span>학습된 일반 문체가 적용됩니다.</span></div>) : (<div className="p-3 bg-gray-900 border border-gray-700 rounded-lg text-xs text-gray-500 flex items-center gap-2"><Info size={14} /> <span>저장된 문체를 선택하거나 설정에서 학습시키세요.</span></div>)}</div>
                </div>
                <div className="space-y-6">
